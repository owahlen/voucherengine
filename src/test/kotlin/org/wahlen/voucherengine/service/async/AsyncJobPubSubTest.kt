...
class AsyncJobPubSubTest @Autowired constructor(
    ...
    class TestConfig {
        ...
        ): VoucherAsyncService = TrackingAsyncService(
    ...
    class TrackingAsyncService(
        asyncJobRepository: AsyncJobRepository,
        voucherService: VoucherService,
        objectMapper: ObjectMapper,
        clock: Clock,
        private val tracker: TestExecutionTracker
    ) : VoucherAsyncService(asyncJobRepository, voucherService, objectMapper, clock) {
    ...
    fun `should publish and process BulkUpdateCommand`() {
        ...
        val jobId = asyncJobPublisher.publishBulkUpdate(tenantName, updates)
    ...
    fun `should publish and process MetadataUpdateCommand`() {
        ...
        val jobId = asyncJobPublisher.publishMetadataUpdate(tenantName, request)
    ...
    fun `should publish and process VoucherImportCommand`() {
        ...
        val jobId = asyncJobPublisher.publishVoucherImport(tenantName, request)
    ...
    fun `should process multiple commands in sequence`() {
        val jobId1 = asyncJobPublisher.publishBulkUpdate(tenantName, listOf(VoucherBulkUpdateRequest(code = "C1", metadata = emptyMap())))
        val jobId2 = asyncJobPublisher.publishMetadataUpdate(tenantName, VoucherMetadataUpdateRequest(codes = listOf("C2"), metadata = emptyMap()))
        val jobId3 = asyncJobPublisher.publishVoucherImport(tenantName, VoucherImportRequest(vouchers = listOf(VoucherCreateRequest(code = "C3", type = "DISCOUNT_VOUCHER"))))
